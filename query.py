...
# "SQLAlchemy"
    db.session.query(Brand.name, Model.name, Brand.headquarters
                    ).join(Model).filter(Model.year == year).all()
    
def get_brands_summary():
    """Prints out each brand name (once) and all of that brand's models,
    including their year, using only ONE database query."""
    # **********db.session.query(Brand.brand_id, Brand.headquarters, Model.name).join(Model).filter(Model.year == year).all()
